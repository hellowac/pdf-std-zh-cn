...
=== "中文"
    ...
    - （PDF 1.2）采样函数(sampled function)（类型 0）使用一组样本值来定义函数。使用各种技术在样本值之间插值；见 [7.10.2](#7102-type-0-采样-函数)，“类型 0（采样）函数。”
...
=== "英文"
    ...
    - (PDF 1.2) A sampled function (type 0) uses a table of sample values to define the function. Various techniques are used to interpolate values between the sample values; see [7.10.2](#7102-type-0-采样-函数), "Type 0 (Sampled) Functions."
